...
fn set_ipv4(ipv4_env: &str) -> Ipv4Addr {
  match std::env::var(ipv4_env) {
    Ok(value) => value
      .parse::<Ipv4Addr>()
      .expect("IPV4 environment variable should be as '127.0.0.1', std::net::Ipv4Addr"), // TODO: IPv4 number is not pure number, but number array.
    Err(error) => {
...
#[cfg(test)]
mod test {
  use crate::{set_ipv4, set_port, DEFAULT_IPV4, DEFAULT_PORT, IPV4_ENV, PORT_ENV};
